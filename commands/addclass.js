...
class AddClassCommand extends Command {
    ...
    async exec(message, args) {
        ...
        if(args.information != null) {
            ...
            if(result == -1) {
                return message.channel.send(error("that class isn't currently in the approved class database. Use $classes to see a list of approved classes.", message));
            } else {
                for(var i = 0; i < arr.length; i++) {
                    if(arr[i].toLowerCase().includes(args.information.toLowerCase())) {
                        return message.channel.send(error("you already have that class.", message));
                    }
                }              
                message.guild.channels.cache.forEach(ch => 
                {
                    if(ch.name == (actualClasses[result].replace(/\s/g, "-").toLowerCase()))
                    ch.updateOverwrite(message.member.user.id, { VIEW_CHANNEL: true, SEND_MESSAGES: true });
                }) 
            ...
            message.channel.send(error("you need to specify a value. Use $classes to see a list of approved classes.", message));
        }
    }
